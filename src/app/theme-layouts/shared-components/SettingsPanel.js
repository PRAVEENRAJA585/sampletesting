...
import KyrosSettings from '@kyros/core/KyrosSettings';
...
import KyrosThemeSchemes from '@kyros/core/KyrosThemeSchemes';
import { useSwipeable } from 'react-swipeable';
import KyrosSvgIcon from '@kyros/core/KyrosSvgIcon';
import themesConfig from 'app/configs/themesConfig';
import { changeKyrosTheme } from 'app/store/kyros/settingsSlice';
import { useDispatch } from 'react-redux';
import KyrosSettingsViewerDialog from './KyrosSettingsViewerDialog';
...
function SettingsPanel() {
  ...
  return (
    <>
      <Root id="kyros-settings-schemes" className="buttonWrapper">
        <Button
        ...
        >
          <span>
            <KyrosSvgIcon size={20}>heroicons-solid:cog</KyrosSvgIcon>
          </span>
        </Button>
        ...
        <Button
          ...
          disableRipple
        >
          <KyrosSvgIcon size={20}>heroicons-outline:color-swatch</KyrosSvgIcon>
        </Button>
      </Root>
      <StyledDialog
        ...
        {...settingsHandlers}
      >
        <KyrosScrollbars className="p-16 sm:p-32">
          <IconButton
            className="fixed top-0 ltr:right-0 rtl:left-0 z-10"
            onClick={handleClose}
            size="large"
          >
            <KyrosSvgIcon>heroicons-outline:x</KyrosSvgIcon>
          </IconButton>
          ...
          <KyrosSettingsViewerDialog className="mt-32" />
        </KyrosScrollbars>
      </StyledDialog>
      <StyledDialog
        TransitionComponent={Transition}
      ...
      >
        <KyrosScrollbars className="p-16 sm:p-32">
          ...
          <KyrosThemeSchemes
            themes={themesConfig}
            onSelect={(_theme) => {
              dispatch(changeKyrosTheme(_theme));
            }}
          />
        </KyrosScrollbars>
      </StyledDialog>
