...
export const appendNavigationItem = (item, parentId) => (dispatch, getState) => {
  ...
  return dispatch(setNavigation(KyrosUtils.appendNavItem(navigation, item, parentId)));
};

export const prependNavigationItem = (item, parentId) => (dispatch, getState) => {
  const navigation = selectNavigationAll(getState());

  return dispatch(setNavigation(KyrosUtils.prependNavItem(navigation, item, parentId)));
};

export const updateNavigationItem = (id, item) => (dispatch, getState) => {
  const navigation = selectNavigationAll(getState());

  return dispatch(setNavigation(KyrosUtils.updateNavItem(navigation, id, item)));
};

export const removeNavigationItem = (id) => (dispatch, getState) => {
  const navigation = selectNavigationAll(getState());

  return dispatch(setNavigation(KyrosUtils.removeNavItem(navigation, id)));
};
...
} = navigationAdapter.getSelectors((state) => state.kyros.navigation);
...
export const selectNavigation = createSelector(
  ...
  (navigation, language, userRole) => {
    ...
    return setTranslationValues(
      _.merge(
        [],
        filterRecursively(navigation, (item) => KyrosUtils.hasPermission(item.auth, userRole))
      )
    );
  }
);
...
export const selectFlatNavigation = createSelector([selectNavigation], (navigation) =>
  KyrosUtils.getFlatNavigation(navigation)
);
